...
export interface $MultipleResults<IEntity> {
  loading: boolean;
  data: IEntity[];
...
export const useReciveAll = (tableName: string): $MultipleResults<IEntity> => {
...
export const useRecive = (
  id: number,
  tableName: string
): $MultipleResults<IEntity> => {
  const db = new MyAppDatabase();
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<IEntity[]>([]);
  const [effect, setEffect] = useState(true);
  ...
  const loadData = useCallback(
    ...
    [setLoading, setData, id, db, cancelPromise]
  );
